...
- 正本の戦略定義は `StrategyBlueprintSpec`
- canonical candidate catalog の正本は blueprint のみ
- `comparison_service` は blueprint / normalized execution context を直接参照する
...
1. `StrategyBlueprintSpec` を導入する
2. `StrategyExecutionPlanSpec` を導入する
3. `StrategySignalSpec` を導入する
4. `legacy StrategySpec -> blueprint` 変換を実装する
5. `blueprint -> legacy StrategySpec` 変換を実装する
6. legacy adapter の互換判定を独立させる
7. `ComparisonSpec` が blueprint を受けられるようにする
8. `comparison_service` が blueprint を正規化して実行できるようにする
9. candidate 定義を `blueprint first` に移行する
10. baseline / canonical catalog を blueprint 正本に寄せる
11. `selection × portfolio_model × execution` の product builder を作る
12. predictor を signal として builder に取り込む
13. `DEFAULT_COMPARISON_SPEC` を blueprint 起点にする
14. API payload で blueprint と互換情報を見えるようにする
...
- ただし runtime はまだ完全な blueprint-native execution model ではない

18. legacy adapter を通さず blueprint を直接評価する経路を作る
状況:
- direct execution path は存在する
- `StrategySpec` の explicit execution context と `executionMode` は導入済み
- `StrategySpec <-> blueprint` の往復でも execution metadata をかなり保持できる
- ただしまだ完全な blueprint-native evaluator ではない
...
- ただし blueprint-native evaluator と完全に一体化した再検証運用にはまだ届いていない

### Not Started

21. `StrategyBlueprintSpec` を最終ドメイン名へリネームする
状況:
- `StrategyBlueprintSpec` は移行期の名前としては機能した
- ただし現在は strategy 定義の正本になっており、`blueprint` という語が中間表現っぽく見える
- 候補は `StrategyDefinition` を第一候補として検討する
- 実施時は API payload / tests / CLI / docs の語彙も一緒に揃える

## Remaining Duplication

いま残っている主な二重構造は次の通り。

- direct execution bridge と blueprint-native execution の二重経路
- runtime 境界に残る最終的な `extensions` fallback
- run store が blueprint fingerprint ではなく legacy payload 依存な点

外部入口の二重化はほぼ解消済み。

- API は `/api/comparison`
- CLI は `comparison-summary`
- candidate catalog の production 正本は blueprint のみ

## Near-Term Next Steps

1. blueprint-native evaluator を進めて、direct execution bridge をさらに薄くする
2. 残っている `extensions` fallback を helper の外から見えない形まで縮める
3. run store を fingerprint-first な検索・再利用モデルへさらに寄せ切る
4. `comparison-run-spec` ベースの再検証運用を blueprint-native evaluator 側へさらに寄せる
